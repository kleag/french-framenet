...
<xsl:preserve-space elements="text sentence"/>
		...
		<head>
			...
			<style type="text/css">
				...
				<xsl:for-each select="fn:header/fn:frame/fn:FE">
                    span.<xsl:value-of select="translate(../../../@frame, '.', '-')"/>-<xsl:value-of select='@name'/> {
						color: #<xsl:value-of select='@fgColor'/>;
						background-color: #<xsl:value-of select='@bgColor'/>;
					}
					span.<xsl:value-of select="translate(../../../@frame, '.', '-')"/>-<xsl:value-of select='@abbrev'/> {
						color: #<xsl:value-of select='@fgColor'/>;
						background-color: #<xsl:value-of select='@bgColor'/>;
					}
				</xsl:for-each>
			...
			<script type="text/javascript">
                ...
                // Corentin Ribeyre: change name of frame when containing dots because
                // css class does not support this char (it is reserved)
		//marie debug 9/5/2016 : keep original name
		origframeName = frameName
				...
				if (mode == "" || mode == "annotation") {
					...
					<xsl:for-each select="fn:header/fn:frame/fn:FE">
                        ...
                        //]]>
                        // Corentin Ribeyre: use modified frameName (dots are replaced by -)
						rowStr += frameName+"-<xsl:value-of select='@name'/>";
						//<![CDATA[
						rowStr += "'>";
						//]]>
                    ...
                    //var cBy = '';
                    ////]]>
		    <!-- @asfalda : no annotator id information-->                    
		    <!--
		    ...
		    -->
                ...
                /* LEXICAL ENTRY MODE */
					...
					// @asfalda : no valence pattern table, so can be less than 60%
					//frameset.setAttribute("rows","60%,*");
					frameset.setAttribute("rows","40%,*");
					...
					function addSent(){
						for (var i=0; i < addSent.arguments.length; i++ ) {
							...
							if (addSentID(sent_id)) {
								...
								top.frames[1].transformAndAdd(sent_id);
							}
						}
					}
                ...
                /* LEXICAL ENTRY MODE BOTTOM SENTENCE FRAME */
					...
					function clearSents() {
						...
						top.frames[0].sentIDs = new Array();
					}
					...
					function removeSent(sent_id) {
						...
						if (nodeC_On && nodeC_Off) {
							...
							top.frames[0].removeSentID(sent_id);
						}
					}
				...
				function navBar() {
                    ...
                    if (mode == "lexentrytop") {
                        ...
                        luLink.setAttribute("target","_top");
		    ...
		    // @asfalda: marie debug 9/5/2016 : link must use origframeName
                    frameLink.setAttribute("href","../frameIndex.xml?frame=" + origframeName + "&banner=" + banner);
                    frameLink.setAttribute("target","_top");
                    frameLink.innerHTML = origframeName;
				...
				function getSentenceAnnotation(sentId) {
					...
					//]]>
                    var frameName = "<xsl:value-of select='@frame'/>";
                    //Corentin Ribeyre: replace dots by - because css does not support using it in class names
                    frameName = frameName.replace('.', '-');
					//<![CDATA[
					...
					<xsl:for-each select="fn:subCorpus">
						...
						//]]>
						<xsl:for-each select="fn:sentence/fn:annotationSet[@status='MANUAL'or@status='AUTO_EDITED'or@status='AUTO_APP']">
                            ...
                            var actualsentId = "<xsl:value-of select='../@ID' />";
							//<![CDATA[
							if (sentId == "all" || cursentId == sentId) {
								...
								<!-- marie candito: display sentid at end of sentence -->
								<xsl:variable name="redundantsentId">
									<xsl:value-of select='../@ID' />
								</xsl:variable>
								sentOrig = "<xsl:value-of select='$processSent' /> (sentid = <xsl:value-of select='$redundantsentId' />) ";
								//<![CDATA[
								...
								<xsl:for-each select="fn:layer[@name='FE'or@name='Target'or@name='Noun'or@name='Adj'or@name='Verb']/fn:label">
									...
									<xsl:if test="@start">
										...
										<xsl:if test="../@name = 'Target'">
											//<![CDATA[
                                            // color
                                            // Corentin Ribeyre: Change the last param of the function (toTheEnd) to true
                                            // to force a better ordering of tags: Target should always be a child and not a parent
                                            // WARN : this should work seamlessly unless a Target spans an entire sentence and there exist FEs
											insertMapValue(charLabelMap[0][0], start, "<TARGET><span class='Target'>", true);
				...
				function showLexEntry(targetDivId) {
					...
					var luDef = "<xsl:value-of select='normalize-space(fn:definition)' />";
					...
					luInfoDiv.innerHTML = "<h1>Lexical Entry</h1>" +
                                             "<h1>" + luName + "</h1>" + "<h3>Frame: " + frName + "</h3>" ;
		    ...
		    luInfoDiv.innerHTML += "<h3>Frame Elements and links to their realizations</h3>" ;
					...
					//<![CDATA[
//@asfalda : no valence patterns for now
//@asfalda					finalStr = "<h3>Valence Patterns:</h3>";
//@asfalda					finalStr += "These frame elements occur in the following syntactic patterns: <p />";
//@asfalda					finalStr += "<table class='feGReal'>";
//@asfalda					finalStr += "<tr><th>Number Annotated</th><th colspan='" + maxFEGroupSize +
//@asfalda						"'>Patterns</th></tr>";
//@asfalda					//]]>
//@asfalda					var numFEs;
//@asfalda					<xsl:for-each select="fn:valences/fn:FEGroupRealization">
//@asfalda						idList = "";
//@asfalda						<xsl:for-each select=".//fn:annoSet">
//@asfalda							idList += "\"<xsl:value-of select='@ID' />\",";
//@asfalda						</xsl:for-each>
//@asfalda						//<![CDATA[
//@asfalda						idList = idList.substring(0,idList.length-1);
//@asfalda						//]]>
//@asfalda						curSum = "<xsl:value-of select='@total'/>";
//@asfalda						//<![CDATA[
//@asfalda						finalStr += "<tr><td class='totals'>";
//@asfalda						finalStr += getAddSentLink(idList) + curSum + "</a> TOTAL";
//@asfalda						finalStr += "</td>";
//@asfalda						var j = 0;
//@asfalda						numFEs = 0;
//@asfalda						//]]>
//@asfalda						<xsl:for-each select="fn:FE">
//@asfalda							//<![CDATA[
//@asfalda							numFEs++;
//@asfalda							//]]>
//@asfalda							fe = "<xsl:value-of select='@name' />";
//@asfalda							//<![CDATA[
//@asfalda							finalStr += "<td><span class='"+frName+"-"+fe+"'>"+fe+"</span></td>";
//@asfalda							//]]>
//@asfalda						</xsl:for-each>
//@asfalda						//<![CDATA[
//@asfalda						for (var i = numFEs; i < maxFEGroupSize; i++){
//@asfalda							finalStr += "<td><br /></td>";
//@asfalda						}
//@asfalda						finalStr += "</tr>";
//@asfalda						//]]>
//@asfalda						<xsl:for-each select="fn:pattern">
//@asfalda							idList = "";
//@asfalda							<xsl:for-each select="fn:annoSet">
//@asfalda								idList += "\"<xsl:value-of select='@ID' />\",";
//@asfalda							</xsl:for-each>
//@asfalda							idList = idList.substring(0,idList.length-1);
//@asfalda							sum = "<xsl:value-of select="@total"/>";
//@asfalda							//<![CDATA[
//@asfalda							finalStr += "<tr><td class='totals'> (";
//@asfalda							finalStr += getAddSentLink(idList) + sum + "</a>";
//@asfalda							finalStr += ") </td>";
//@asfalda							numFEs = 0;
//@asfalda							//]]>
//@asfalda							<xsl:for-each select="fn:valenceUnit">
//@asfalda								//<![CDATA[
//@asfalda								numFEs++;
//@asfalda								//]]>
//@asfalda								pt = "<xsl:value-of select='@PT' />";
//@asfalda								gf = "<xsl:value-of select='@GF' />";
//@asfalda								//<![CDATA[
//@asfalda								if (gf == ""){
//@asfalda									gf = "--";
//@asfalda								}
//@asfalda								finalStr += "<td>" + pt + "<br />" + gf + "</td>";
//@asfalda								//]]>
//@asfalda							</xsl:for-each>
//@asfalda							//<![CDATA[
//@asfalda							for (var i = numFEs; i < maxFEGroupSize; i++){
...
//@asfalda					</xsl:for-each>
//@asfalda					//<![CDATA[
//@asfalda					finalStr += "</table>";
//@asfalda
//@asfalda					var patternTable = document.createElement('div');
//@asfalda					patternTable.setAttribute('id', 'patternTable');
//@asfalda					patternTable.innerHTML = finalStr;

					var targetDiv = document.getElementById(targetDivId);
					targetDiv.appendChild(luInfoDiv);
					targetDiv.appendChild(feTable);
					// @asfalda : no pattern table for now
