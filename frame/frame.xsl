	...
	<head>
		...
		function convertDefinition(def) {
			...
			for (var j = 0; j < defWords.length; j++) {
				...
				if (defWords[j]) {
                    ...
                    if (defWords[j].substr(0,4) == '&lt;') {
                        ...
                        if (tag.charAt(0) != '/') {
                            ...
                            else if (tag == 'asf') { // @asfalda : formatting of asfalda comments in definitions
			         newDef += '<font style="font-weight:bold; font-size:large;">';
                                 //newDef += '<font style="font-weight:bold; color: rgb(255, 0, 0); text-transform:uppercase;">';
				 //newDef += <h3>; // h3 ne marche pas...
                            }
			    ...
			    // @asfalda : add url to other frames cited in definition
                            else if (tag == 'frframe') { // fr frame formatting
                                // go to the frame name
                                while (!defWords[j] || defWords[j] == '') j++;
                                frameName = defWords[j];
                                    newDef += '<a href="' + frameName + '.xml">' + frameName
                            }
                            ...
                            if (tag == 't' || tag == 'fex' || tag == 'fen' ) { //@asfalda: close also for asf tags
                                // for the target and fe coloring, just close the font tag
                                newDef += '</font>';
							}
                            else if (tag == 'asf' ) { //@asfalda: close also for asf tags
                                newDef += '</font>';
							}
			    // @asfalda : close link to other frame
                            else if (tag == 'frframe') { // fr frame formatting
                                newDef += '</a>'
                            }
...
<!-- @asfalda: remove new line after h3-->
<!--<style media="all">
...
-->
   	...
   	<body>
        ...
        <div id='bodyDelay' style='display:none;'>
		  ...
		  <!-- @asfalda: add a link to all annotations of the frame -->
		    <table>
		      <tr>
			<xsl:variable name='framename' select='@name' />
			<td><h3><a href="../allanno/allanno_{$framename}.xml" target='_top'>All annotations of this frame</a></h3></td>
			<td></td>
			<td></td>
			<td><h3><a href="../luIndex.xml" target='_top'>Lexical Unit Index</a></h3></td>
		      </tr>
		    </table>
                </div>
            </xsl:if>
	    ...
	    <font style="font-weight:bold; font-size:large;">Domains: </font> <xsl:value-of select='@domains'/>
            <h3>Definition:</h3>
...
<!--@asfalda: condition included in select-->	      
<!--@asfalda: we choose to display core-unexpressed as core -->
                <xsl:for-each select='fn:FE'>
                    <xsl:sort select='@name' order='ascending' />
                    <xsl:variable name='feNum' select='position()' />
                    <xsl:if test='@coreType="Core" or @coreType="Core-Unexpressed"' >
                        <tr>
		...
		<!--@asfalda : heading displayed only if there are some-->
		<xsl:if test='fn:FE[@coreType!="Core" and @coreType!="Core-Unexpressed"]'>
                  <tr><td><h3>Annotated Non-Core FEs:</h3></td></tr>
		</xsl:if>
		<!-- don't add condition in the for-each, to get unique feNum-->
                <xsl:for-each select='fn:FE'>
                    <xsl:sort select='@name' order='ascending' />
                    <xsl:variable name='feNum' select='position()' />
<!--@asfalda: we choose to display core-unexpressed as core -->
...
<!--@asfalda: display the name of the relation iff there are some relation instances-->		  
<!--                    <xsl:value-of select='@type' />:-->
                    <xsl:variable name='frameRelation' select='@type' />
                    <xsl:for-each select='fn:relatedFrame'>
                        <xsl:sort select='.' order='ascending' />
			<xsl:if test='position()=1'>
			  <xsl:value-of select='$frameRelation' />:
			</xsl:if>
                        <xsl:variable name='rfName' select='current()' />
			<xsl:choose>
			  <xsl:when test="starts-with($frameRelation, 'Relation to')">
...
<!--@asfalda            <p>Created by <xsl:value-of select='@cBy' /> on <xsl:value-of select='@cDate' /></p> -->
            <hr />
            <xsl:if test="$internalMode!='desktop'">
                <table class='sortable' cellspacing='10' margin-left='auto' margin-right='auto'>
                    <tr><td><a href="javascript:"><b>Lexical Unit</b></a></td>
<!--@asfalda                        <td><a href="javascript:"><b>LU Status</b></a></td> -->
                        <td><a href="javascript:"><b>Lexical Entry</b></a></td>
                        <td><a href="javascript:"><b>Corpus Annotation</b></a></td>
                        <td><a href="javascript:"><b>Annotation of this lemma in other frames</b></a></td>
<!--@asfalda                        <td><a href="javascript:"><b>Annotator ID</b></a></td> -->
<!--@asfalda                        <td><a href="javascript:"><b>Created Date</b></a></td> -->
                    </tr>
                    <xsl:for-each select='fn:lexUnit'>
                        <xsl:sort select='@name' order='ascending' />
                        <tr><td><xsl:value-of select='@name' /></td>
<!--@asfalda                        <td><b><xsl:value-of select='@status' /></b></td> -->
                        <xsl:variable name='luID' select='@ID' />
<!--@asfalda: we remove the filter on status 
...
-->
                        <td><xsl:if test='fn:sentenceCount/@annotated>0'>
                            <a href='../lu/lu{$luID}.xml?mode=lexentry' target='_top'>Lexical entry</a>
                        </xsl:if></td>
                        <td>
			  <xsl:if test='fn:sentenceCount/@annotated>0'>
                            <a href='../lu/lu{$luID}.xml?mode=annotation' target='_top'>Annotation</a>
                        </xsl:if>
<!--@asfalda: for LU inhibited in lexicon -->
			  <xsl:if test='fn:sentenceCount/@annotated=0 and @inhib_in_lexicon="true"'>
			    Sense not found in corpus (inhib)
                        </xsl:if>
<!--@asfalda: LU whose lemma has some annotations (for other frame), but not for this frame -->
			  <xsl:if test='fn:sentenceCount/@annotated=0 and @inhib_in_lexicon!="true" and @lemma_is_annotated="true"'>
			    Sense not found in corpus
                        </xsl:if>
<!--@asfalda: for non-annotated LUs, show the number of occurrences of the lemma.cat of the LU in the target corpus (=number of potential occurrences)-->
			  <xsl:if test='fn:sentenceCount/@annotated=0 and @inhib_in_lexicon!="true" and @lemma_is_annotated!="true" and @nbocc>0'>
                            Lemma not annotated yet (<xsl:value-of select='@nbocc'/> occ. of this lemma.cat)
                        </xsl:if>
<!--@asfalda: for 0 occurrence LUs-->
			  <xsl:if test='fn:sentenceCount/@annotated=0 and @inhib_in_lexicon!="true" and @lemma_is_annotated!="true" and @nbocc=0'>
                            Lemma not found in corpus
                        </xsl:if>
			</td>
<!--@asfalda                        <td><xsl:value-of select='@cBy' /></td> -->
<!--@asfalda                        <td><xsl:value-of select='@cDate' /></td> -->
                        <td>
                        <xsl:for-each select='fn:otherlexUnit'>
                          <xsl:sort select='@framename' order='ascending' />
                          <xsl:variable name='otherluID' select='@ID' />
			  <xsl:if test='@annotatedsent>0'>
                            <xsl:text> </xsl:text>/<xsl:text> </xsl:text><xsl:text> </xsl:text><a href='../lu/lu{$otherluID}.xml?mode=annotation' target='_top'><xsl:value-of select='@framename'/></a>
                          </xsl:if>
...
<!--@asfalda                       <td><b>LU Status</b></td> -->
<!--@asfalda                         <td><b>Annotator ID</b></td> -->
<!--@asfalda                         <td><b>Created Date</b></td> -->
                    </tr>
                    <xsl:for-each select='fn:lexUnit'>
                        <xsl:sort select='@name' order='ascending' />
                        <tr><td><xsl:value-of select='@name' /></td>
