...
<xsl:if test='$runXSL!=""'>
    ...
    <xsl:variable name="lcletters">aàbcdeéèêfghiîjklmnoôpqrstuùûvwxyz</xsl:variable>
    <xsl:variable name="ucletters">AÀBCDEÉÈÊFGHIÎJKLMNOÔPQRSTUÙÛVWXYZ</xsl:variable>
<!-- @asfalda: print as table -->
    <table class='sortable' cellspacing='10' margin-left='auto' margin-right='auto'>
      <tr> <td><b>LU</b></td> <td><b>Frame</b></td> <td><b>status</b></td> </tr>
	...
	<!-- @asfalda : Other_sense should be displayed iff there are some annotated Other_sense occurrences -->
	<xsl:if test='@frameName!="Other_sense" or ($hasAnno="true")'>
	  <tr>
	    <td><xsl:value-of select='$name' /></td>
            <td><a href='frameIndex.xml?frame={$frName}'><xsl:value-of select='$frName' /></a></td>
	    ...
	    -->
	    <td>
	      <xsl:if test='@inhib_in_lexicon="true"'>
		Sense not found in corpus (inhib)
              </xsl:if>
              <xsl:if test='@inhib_in_lexicon!="true" and @nbocc=0'>
		<xsl:text>    Lemma not found in corpus</xsl:text>
              </xsl:if>
              <xsl:if test='@inhib_in_lexicon!="true" and @nbocc>0 and @lemma_is_annotated="false"'>
		<xsl:text>    Lemma.cat not annotated yet</xsl:text>
              </xsl:if>
              <xsl:if test='@inhib_in_lexicon!="true" and @nbocc>0 and @lemma_is_annotated="true" and $hasAnno="true"'>
		<xsl:text>    </xsl:text>
		<a href='lu/lu{$id}.xml?mode=lexentry'>Lexical Entry</a>
		<xsl:text>    </xsl:text>
		<a href='lu/lu{$id}.xml?mode=annotation'>Annotation</a>
              </xsl:if>
              <xsl:if test='@inhib_in_lexicon!="true" and @nbocc>0 and @lemma_is_annotated="true" and $hasAnno="false"'>
		<xsl:text>    Sense not found in corpus</xsl:text>
              </xsl:if>
	    </td>
          </tr>
	</xsl:if>
...
<html>
	...
	<body>
        ...
        <h1>French FrameNet Index of Lexical Units</h1><p>This page is an index to alphabetical lists of the lemmas of the lexical units (LUs). </p><p>Each LU whose lemma starts with the entered string is displayed with its lemma, part-of-speech and frame. </p><p>If some annotations are available for the LU, it will be followed by links to the HTML files for the lexical entry and the annotated sentences. </p><p>
...
<!--         | <a href='javascript:getLUs("Ô")'>Ô</a> -->
...
<!--         | <a href='javascript:getLUs("Ù")'>Ù</a> -->
<!--         | <a href='javascript:getLUs("Û")'>Û</a> -->
